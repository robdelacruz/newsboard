...
INSERT INTO entryvote (entry_id, user_id) VALUES (1, 2);
INSERT INTO entryvote (entry_id, user_id) VALUES (1, 3);
INSERT INTO entryvote (entry_id, user_id) VALUES (1, 4);
...
', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 2, 0);
INSERT INTO entryvote (entry_id, user_id) VALUES (2, 1);
INSERT INTO entryvote (entry_id, user_id) VALUES (2, 3);

-- comment entries
INSERT INTO entry (entry_id, thing, title, body, createdt, user_id, parent_id) VALUES (3, 1, '', 'I thought I read somewhere that NewsBoard was programmed in Go, with Sqlite3 as the database. Their code is even posted on github.', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 3, 2);
INSERT INTO entry (entry_id, thing, title, body, createdt, user_id, parent_id) VALUES (4, 1, '', 'I bet they used Perl because the UI looks cool.', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 4, 2);
INSERT INTO entry (entry_id, thing, title, body, createdt, user_id, parent_id) VALUES (5, 1, '', 'No, they used Go. Check out their [github](https://github.com/robdelacruz/newsboard) page.', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 3, 4);

INSERT INTO entry (entry_id, thing, title, body, createdt, user_id, parent_id) VALUES (6, 1, '', 'NB''s interface looks oddly familiar. It''s like I saw it somewhere else before.', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 1, 2);

INSERT INTO entry (entry_id, thing, title, body, createdt, user_id, parent_id) VALUES (7, 1, '', 'That''s because they stole the entire thing from HackerNews.', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'), 2, 6);
